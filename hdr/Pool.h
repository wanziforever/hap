// C++ Standard component, Release 3.0
...
class Block_pool {
  ...
  unsigned cnt;
  Block_header *head;
  Block_pool(const Block_pool&); // copy protection
  void operator=(const Block_pool&); // copy protection
public:
...
struct Vpool_stats {
  ...
  size_t mem_inuse; // bytes currently bolding elements
...
class Vpool:private Block_pool {
  ...
  void operator=(const Vpool&);
  void grow();
  void do_thrink(void *, size_t);
