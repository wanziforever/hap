...
int CRlogFile::writeOM(const char* str, int str_len) {
...
*/
int CRlogFile::getOM(CRlogLoc& externalLoc, char outbuf[], int outbufsize) {
...
void CRlogFile::getLogicalName(CRlogFname& result_str) const {
...
*/
int CRlogFile::readLog(std::string& inbuf) {
  CRlogFname filename;
	int totalBytes = 0;
	for (int i = 0; i < maxFiles; i++) {
		curLocation.getSubFileName(i, filename);
