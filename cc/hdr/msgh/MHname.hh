...
inline MHname::MHname() {
  name[MHmaxNameLen] = '\0';
}

inline MHname::MHname(const Char *ptr) {
  strncpy(name, ptr, MHmaxNameLen);
  name[MHmaxNameLen] = '\0';
}

// Assgin a MHname object to another MHname to object
inline MHname& MHname::operator=(const MHname &mhname) {
  strncpy(name, mhname.name, MHmaxNameLen+1);
  return (*this);
}

// Assign a character string to a MHname object
inline MHname& MHname::operator=(const Char *ptr) {
  strncpy(name, ptr, MHmaxNameLen);
  name[MHmaxNameLen] = '\0';
  return (*this);
}
...
inline Short MHname::operator==(const MHname& mhname) const {
  return (strcmp(name, mhname.name) == 0);
}

// Returns 1 fi both objects are equal; return 0 otherwise
inline Short MHname::operator==(const Char *ptr) const {
...
// Returns non-zero if two objects are not equal; returns 0 otherwisze
inline Short MHname::operator!=(const Char *ptr) const {
...
// programmer to the sure the buffer is large enough
inline Char* MHname::dump(Char *ptr) const {
